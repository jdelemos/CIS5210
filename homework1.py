...
python_concepts_question_1 = "Python is a language that is both strong and dynamically typed. Dynamic typed languages allow  for the variable type to be determined at run time - programmers don't explicitly assign type while coding like in Java or C. Strong typing means that variables don't automatically change type when interacted with. This means that in python while some variables might hold string values of numbers, they can be reassigned to integers later in the program without causing an error. The compiler will automatically handle the underlying memory mangagement. This is a huge advantage to programmers who prefer flexibility when working with their variables."

python_concepts_question_2 = "Python dictionary keys must be hashable, and immutable. To correct the example you must swap the key and data positions."

python_concepts_question_3 = "The second function is significantly faster because strings are immutable. We see each loop iteration as a n time moving through a dataset. The second is a integer result, as there is no loops occuring."
...
def extract_and_apply(lst, p, f):
    return [f(x) for x in lst if p(x) == True]

#item for row in matrix, thats the rows, then for item in row is the individual 
...
feedback_question_1 = """
Approximately eight to ten hours. I wrote all the code myself and tried to learn as much as possible.
"""

feedback_question_2 = """
The first question that proved difficult was transposing the matrix. Conceptually, I understand it's simply turning rows into columns, but coding that proved to be difficult. The other question that gave me trouble was translating the polynomial back into english. There were a ton of edge cases that gave me problems. It was difficult to find all of them. After some time I managed to find them all. 
"""

feedback_question_3 = """
This was a useful assignment. It was very helpful to refresh myself with some of the tools needed to parse data in python.  If I could modify it, I would advocate for one simple question in each section. Allow us to gain momentum, then give us questions of increasing difficulty.   
"""
